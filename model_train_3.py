...
def custom_collate_fn(batch):
    max_len = max([len(frames) for frames, label in batch])
    padded_batch = []
    for frames, label in batch:
        if len(frames) < max_len:
            # Pad the sequence to match the max length
            pad = max_len - len(frames)
            padding = torch.zeros(pad, *frames[0].shape)
            frames = torch.cat([frames, padding], dim=0)
        padded_batch.append((frames, label))
    frames_batch = torch.stack([item[0] for item in padded_batch])
    labels_batch = torch.tensor([item[1] for item in padded_batch])
